...
public class WrappedBeastTree {
	...
	private BitSet initCladeBitSet(Node vertex) {
		...
		if (vertex.isLeaf()) {
			...
			BitSet firstChildBitSet = initCladeBitSet(vertex.getChild(0));
			BitSet secondChildBitSet = initCladeBitSet(vertex.getChild(1));
			cladeAsBitSet.or(firstChildBitSet);
			cladeAsBitSet.or(secondChildBitSet);
		}

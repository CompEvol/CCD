...
public abstract class AbstractCCD implements ITreeDistribution {
	 ...
	 * 
	 * @param trees
	 *            the trees used to build and populate the CCD graph of this
	 *            {@link AbstractCCD}
	 * @param burnin
	...
	public AbstractCCD(List<Tree> trees, double burnin) {
		...
		if (burnin == 0) {
			...
			for (Tree tree : trees.subList(numDiscardedTrees, trees.size())) {
				treesToUse.add(tree);
			}
		}
	 ...
	 * 
	 * @param treeSet
	 *            an iterable set of trees, which contains no burnin trees and
	 ...
	 * 
	 * @param tree
	 *            to be added and processed into the CCD graph
	 */
	 ...
	 * 
	 * @param tree
	 *            to be taken out from set of trees this CCD is based on
	 * @param tidyCCDGraph
	 *            whether to keep the CCD graph tidy, that is, not keeping any
	...
	private Clade reduceCladeCount(Node vertex) {
		...
		if (!vertex.isLeaf()) {
			...
			if (currentPartition.getNumberOfOccurrences() == 0) {
				currentClade.partitions.remove(currentPartition);

				if (currentClade.partitions.size() == 0) {
					this.cladeMapping.remove(cladeInBits);
				}
			}
		}
	...
	/**
	 * Tidy up the this CCD by removing nontrivial clades that have no clade
	...
	public boolean tidyUp() {
		...
		System.out.println("tidying up ccd");
		...
		for (Clade clade : this.getCladeMapping().values()) {
			if (clade.isLeaf()) {
				...
				System.out.println("- clade found that "
						+ (clade.getParentClades().isEmpty() ? "has no parents: "
								: "has no partitions: ")
		...
		while (!cladesToRemove.isEmpty()) {
			...
			for (CladePartition partition : cladeToRemove.getPartitions()) {
				...
				firstChild.getParentClades().remove(cladeToRemove);
				if (firstChild.getParentClades().isEmpty()) {
					cladesToRemove.add(firstChild);
				}

				secondChild.getParentClades().remove(cladeToRemove);
				if (secondChild.getParentClades().isEmpty()) {
					cladesToRemove.add(secondChild);
				}
			}
		}
	...
	protected abstract void tidyUpIfDirty();
	...
	/* Helper method. */
	protected void resetCachedValuesIfProbabilitesDirty() {
	...
	public Tree getMAPTree() {
		return this.getMAPTree(HeightSettingStrategy.None);
	}
	...
	private Node getVertexBasedOnStrategy(Clade clade, SamplingStrategy samplingStrategy,
		...
		if (clade.isLeaf()) {
			...
			String taxonName = baseTrees.get(0).getNode(leafNr).getID();
	 ...
	 * 
	 * @param tree
	 *            whose average RF distance we compute
	 ...
	 * 
	 * @param mask
	 *            specifies which taxa the names are requested for

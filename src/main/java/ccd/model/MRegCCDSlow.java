...
/**
 * The reference implementation of {@link MRegCCD}: identical model, boundary counts obtained by
 * direct recursive enumeration rather than by indexing. Retained so that the faster implementation
 * can be checked against it (see {@code MRegCCDAgreementTest}); prefer {@link MRegCCD} in use, which
 * computes the same counts without the enumeration blow-up at boundary 4.
 *
 * <p>MRegCCDSlow -- the one-parameter "per-new-split" regularised CCD. It unifies RegCCD's split-expansion
 ...
 */
public class MRegCCDSlow extends CCD1 {
     ...
     * Boundary 4 matches {@link KRegCCD}'s default reserve ({@code k = 2}, boundaries 3 and 4), so
     * the two models look equally far past the CCD graph. It was 5, which went a boundary further
     * than KRegCCD while the op budget below silently truncated the enumeration before reaching it.
     */
    public static final int DEFAULT_RESERVE_DEPTH = 4;
    ...
    public MRegCCDSlow(List<Tree> trees, double burnin, double mu) {
        this(trees, burnin, mu, DEFAULT_RESERVE_DEPTH, true);
    }

    public MRegCCDSlow(List<Tree> trees, double burnin, double mu, int reserveDepth, boolean useTail) {
        super(trees, burnin);
    ...
    public MRegCCDSlow(TreeSet treeSet, double mu) {
        this(treeSet, mu, DEFAULT_RESERVE_DEPTH, true);
    }

    public MRegCCDSlow(TreeSet treeSet, double mu, int reserveDepth, boolean useTail) {
        super(treeSet);
        validate(mu, reserveDepth);
        this.mu = mu;
        this.reserveDepth = reserveDepth;
    ...
    /**
     * Builds an MRegCCDSlow on {@code trees} with {@code mu} selected by maximising cross-validated
     ...
     */
    public static MRegCCDSlow withOptimisedMu(List<Tree> trees) {
        double mu = ccd.algorithms.regularisation.MRegCCDParameterOptimiser.optimiseMu(trees).mu();
        return new MRegCCDSlow(trees, 0.0, mu);
    }
    ...
    public String toString() {
        return "MRegCCDSlow [mu = " + mu + ", reserveDepth = " + reserveDepth + ", tail = " + useTail
    ...
    /** Always true: MRegCCDSlow is full support, so every tree on this taxon set has positive probability. */
    @Override
     ...
     */
    int countAllNovelResolutions(BitSet C, BitSet[] parts) {
        int k = parts.length;
        if (k == 1) {
            return 1;
        }
    ...
    List<BitSet> subclades(BitSet C) {
    ...
    private List<BitSet> sortedCladeBits() {
        if (sortedCladeBits == null) {
            ...
            all.sort(MRegCCDSlow::compareBitSets);
            sortedCladeBits = all;
        }
    ...
    public Tree sampleTree(HeightSettingStrategy heightStrategy) {
        throw new UnsupportedOperationException(
                "MRegCCDSlow materialised-tree sampling is not implemented; sampleTreeLogProbability() "
